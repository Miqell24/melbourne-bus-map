...
// Melbourne: ONE bundle from Transport Victoria — the PTV "GTFS Schedule"
// (discover.data.vic.gov.au/dataset/gtfs-schedule, CC BY 4.0, weekly) ships
// the whole state as numbered sub-feeds, one google_transit.zip each. This
// map takes four of them: 4 (buses — the metropolitan network as route_type
// 3, and in the same folder the regional town buses of Geelong, Bendigo and
// the Latrobe Valley as 701, which stay out), 11 (SkyBus, the airport
// expresses), 3 (Yarra Trams) and 2 (Metro Trains, route_type 400, with the
// line colours the feed itself ships). Four modes: buses navy, trams family
// red, and the sixteen train lines in Metro's own group colours with the
// trunk treatment — wide ribbon, station discs, always-on names.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|96,Belgrave]
...
// Exact-match repairs for poles a feed names badly. PTV needs none so far:
// the names are the street-corner pairs the flags carry ("Dole Ave/Cheddar
// Rd"), trams add their stop number ("Glenferrie Rd/Wattletree Rd #45"),
// stations are "Jordanville Station".
const NAME_FIX = {};
...
// BRT, Cairo's paratransit) — unused in Melbourne, kept so the engine code
...
// ONE bundle, FOUR sub-feeds, FOUR cfgs.
//
// PTV numbers its sub-feeds by mode: 1 regional trains (V/Line), 2 Metro
// Trains, 3 trams, 4 buses, 5 regional coaches, 6 regional buses, 10
// interstate, 11 SkyBus. The map is metropolitan Melbourne as PTV itself
// draws it — myki zones 1 and 2 — which is exactly the route_type 3 slice of
// sub-feed 4: Melton and Sunbury in the west and north, Whittlesea and
// Kinglake, Healesville and Warburton in the east, Gembrook and Pakenham,
// and the Mornington Peninsula down to Portsea and Flinders. The 701s in the
// same folder are the town networks of Geelong, Ballarat, Bendigo, Seymour
// and the Latrobe Valley, 60–150 km out; sub-feed 6 is the same kind of thing
// for the smaller towns (Kyneton, Gisborne, Lancefield). Neither is Melbourne.
//
// Cut deliberately:
//  * bus 684 Ringwood – Eildon: the one metropolitan-coded line that leaves
//    the region — 110 km out to Lake Eildon, and it alone would stretch the
//    frame 40 km north-east into the ranges (the rule: no stop farther than
//    80 km from Flinders Street; every other line fits — Flinders 72 km,
//    Warburton 71 km, Portsea 63 km);
//  * the V/Line regional trains (sub-feed 1) and coaches (5) — the state;
...
//    Street round trip, 20 trips a week): it is the City Loop that every
//    Burnley, Clifton Hill, Northern and Caulfield group line already draws,
//    not a line of its own, and PTV's own map does not show it.
//
// LINE KEYS: bus numbers are unique across the metropolitan network — the
// repeated short names (200 ×3, 903 ×4, 663 ×4) are ONE line published once
// per contracted operator (the 14-/17-/33- prefixes of route_id), so they
// merge on the shared key by themselves. Trams are their numbers, trains the
// names the platform signs and PTV's maps use ("Belgrave", "Glen Waverley").
// SkyBus ships no short names at all — the five routes are keyed by the
// airport-express brand PTV prints on them (see SKYBUS).
const LBL = new Map();

// SkyBus: five airport expresses with an empty route_short_name — keyed by
// the route_id token and named the way the buses are branded.
const SKYBUS = {
  '11-SKY-vpt-1': 'SkyBus',                    // City (Southern Cross) – Melbourne Airport
  '11-box-vpt-1': 'SkyBus Box Hill',           // "Eastern Express"
  '11-SK4-vpt-1': 'SkyBus Frankston',          // "Frankston & Peninsula Express"
  '11-SUN-vpt-1': 'SkyBus Sunshine',           // "Western Express"
  '11-Ska-vpt-1': 'SkyBus Avalon',             // City – Avalon Airport
};

// The train colours come from the feed itself — PTV fills route_color on
// every Metro line, and they are the group colours of its network map: navy
// 152C6B for the Burnley group (Alamein, Belgrave, Glen Waverley, Lilydale),
// red BE1014 Clifton Hill (Hurstbridge, Mernda), yellow FFBE00 Northern
// (Craigieburn, Upfield), cyan 34ACE1 Sunbury and the Dandenong pair
// (Cranbourne, Pakenham), green 028430 Frankston and Stony Point, pink
// F178AF Cross-City (Sandringham, Werribee, Williamstown), grey 95979A the
// Flemington Racecourse race-day line. Trams keep the family red: the feed
// ships Yarra Trams' route colours too, but colour means the MODE here and
// the tram colours are a wayfinding scheme of their own, not a group code.
const TRAIN_ROUTES = await readCsv(join(ROOT, 'data/gtfs-train/routes.txt'));
const MEL_COLORS = {};
for (const r of TRAIN_ROUTES) {
  const k = (r.route_short_name || '').trim();
  if (!k || k === 'Replacement Bus' || k === 'City Circle') continue;
  if (/^[0-9A-F]{6}$/i.test(r.route_color || '')) MEL_COLORS[k] = '#' + r.route_color.toUpperCase();
}
const isRailTrunk = (l) => l in MEL_COLORS;

// --tram feeds two rail cfgs here, told apart by the name itself: a train
// line's name is a train, everything else (1–96) a tram
const trainSel = tramLines.filter((l) => isRailTrunk(l));
const tramSel = tramLines.filter((l) => l !== 'all' && !isRailTrunk(l));

// the lines that leave the metropolitan frame (see the header)
const FAR_BUS = new Set(['684']);

// Stop names as the flags carry them. The feed writes a stray trailing space
// here and there ("Central Walk Shopping Centre/High St "), nothing else.
const fixName = (n) => n.replace(/\s+/g, ' ').trim();

const MODES = [{
  mode: 'bus', label: 'buses', graphMode: 'road',
  // Melton to Warburton, Sunbury to Flinders — 5 × 5 tiles cut out of the
  // Geofabrik extract (see pipeline/pbf-tiles.py); merged at load, ways
  // deduped by id
  osmFiles: Array.from({ length: 25 }, (_, i) => `data/osm/tiles/t${i + 1}.json`),
  color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['200']),
  feeds: [
    { tag: 'ptv', dir: 'data/gtfs-bus', routeTypes: ['3'],
      skipRoute: (r) => FAR_BUS.has((r.route_short_name || '').trim()),
      mapKey: (sn) => sn || null, nameFix: fixName },
    { tag: 'sky', dir: 'data/gtfs-skybus', routeTypes: ['3'],
      mapKey: (sn, r) => SKYBUS[r.route_id] || null, nameFix: fixName },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
if (tramAll || tramSel.length) MODES.push({
  // Yarra Trams: 24 routes, the largest tram network there is, all on
  // railway=tram — including the reserved-track stretches along St Kilda
  // Rd, Dandenong Rd and the old railway alignments of 96 and 109
  mode: 'tram', label: 'trams', osmFile: 'data/osm/melbourne-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel,
  feeds: [
    { tag: 'yt', dir: 'data/gtfs-tram', routeTypes: ['0'],
      mapKey: (sn) => sn || null, nameFix: fixName, noFeedColors: true },
  ],
});
if (tramAll || trainSel.length) MODES.push({
  // Metro Trains ride mainline rail, so their slice of the graph is
  // railway=rail; allMetro gives every line the trunk treatment without an
  // M-prefix to key off. No allVariants: the feed's stops are PLATFORMS
  // (stop_id per platform under a parent station), so every pattern that
  // differs only by the platform used would count as a branch of its own —
  // the representative rule draws each line's longest regular pattern.
  mode: 'tram', label: 'trains', osmFile: 'data/osm/melbourne-rail.json',
  graphMode: 'tram', railKeep: new Set(['rail']),
  allMetro: true,
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : trainSel,
  feeds: [
    { tag: 'mt', dir: 'data/gtfs-train', routeTypes: ['400'],
      skipRoute: (r) => !isRailTrunk((r.route_short_name || '').trim()),
      mapKey: (sn) => sn || null, lineColor: (k) => MEL_COLORS[k], nameFix: fixName },
  ],
});
