...
build.mjs cannot tell the difference. Melbourne's frame is 120 × 120 km
(Melton to Warburton, Sunbury to the Mornington Peninsula), which the public
Overpass mirrors do not serve in one go; Geofabrik has no Victoria extract, so
the whole country is read once (960 MB, pyosmium with an in-memory node
index) and every tile is written in the same pass.
...
PBFS = [os.path.join(ROOT, 'data', 'australia-latest.osm.pbf')]

# must match pipeline/download.sh and build.mjs
S, N, W, E = -38.50, -37.42, 144.44, 145.80
RAIL_BOX = (-38.42, 144.60, -37.53, 145.56)
...
rail_file = os.path.join(ROOT, 'data/osm/melbourne-rail.json')
...
GEN = 'pbf-tiles.py (Geofabrik australia)'
